...
system_message = """You will translate strings kept in a json formatted input at the end of this prompt delimited by #### and generate a json formatted output. The json file holds a first key with the source language to be translated as a ISO 639-1 language code and all expressions to be translaged in the source language. Below the source language follows a list of target languages to be translated. Each target language holds a list of keys from the source language object to be translated.
    ...
    for de translate: the expressions: "Welcome" and "Menu", for french translate: "Welcome"
